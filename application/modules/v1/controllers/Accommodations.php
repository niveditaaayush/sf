...
class Accommodations extends REST_Controller {
            ...
            if(@sizeof($accommodation) > 0){
                ...
                if (@sizeof($getZone) != 1) {
                    ...
                    if ( ! empty($_FILES['accommodation_photo']['name'])) {
                        $filename = time() . clear_string($_FILES['accommodation_photo']['name']);
    ...
    public function profile_get() {
        ...
        if (@sizeof($accommodation) == 1) {
            ...
            $accommodation->todayCheckins = $this->accommodations_model->getAllTodayCheckins($accommodation->id, DATE);
            $accommodation->totalCheckins = $this->accommodations_model->getAllCheckins($accommodation->id);
            $accommodation->filledRooms   = 0;
