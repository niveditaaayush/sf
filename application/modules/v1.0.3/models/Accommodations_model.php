...
class Accommodations_model extends CI_Model {
    ...
    // admin signin added by nivedita
    ...
    public function getAdminnInfo($aWhere) {
        if ( ! is_array($aWhere) || empty($aWhere)) {
            return array();
        }
        return $this->getOneRecord("admins", $aWhere);
    }
    // end admin info
    ...
    public function deleteAccommodation($aWhere = array()) {
     ...
     {
        $this->db->select_sum('noOfRooms'); 
        $this->db->from($this->tablename);
        $this->db->where(array('status'=>1));
        $count = $this->db->get()->row()->noOfRooms;
       // print_r($count); exit;
        return $count;
     }
    ...
    public function sendemail($sendmail = '',$otp='') {
        ...
        $config['smtp_user']    = 'info.safecheckin@gmail.com';
        $config['smtp_pass']    = 'safecheckin123';
        ...
        $this->email->from('info.safecheckin@gmail.com', 'safecheckin');
        $this->email->to($sendmail); 

        $this->email->subject('VMS OTP');
