...
//    }
    ...
    public function countAllActiveVisitors($date = null, $accommodationId = null,$a_where=array()) {
        ...
        if(!empty($a_where))
        {
            if(!empty($a_where['city']))
                $this->db->where('vms19_accommodations.city',$a_where['city']);
            if(!empty($a_where['SHOArea']))
                $this->db->where('vms19_accommodations.SHOArea',$a_where['SHOArea']);
            if(!empty($a_where['zone']))
                $this->db->where('vms19_accommodations.zone',$a_where['zone']);
            if(!empty($a_where['circle']))
                $this->db->where('vms19_accommodations.circle',$a_where['circle']);
        }
        //$this->db->where('vms19_accommodations.status',1);
        $this->db->where('vms19_visitors_primary_info.status', 1);
        $this->db->join($this->tablename, 'vms19_visitors_primary_info.id = vms19_visitors.vpId');
        $this->db->join('vms19_accommodations', 'vms19_accommodations.id = vms19_visitors_primary_info.accommodationId');
        $count = $this->db->count_all_results($this->visitorstable);
...
//    }
    ...
    public function countAllActiveCheckins($date = null, $accommodationId = null,$a_where = array()) {
...
// added for filters.
    ...
    {
        $query = "select v.firstName , v.lastName , v.idProofType , v.idProofNumber , v.vehicleNumber  from vms19_visitors v join vms19_visitors_primary_info pi on v.vpId = pi.id where pi.status = 1 and idProofNumber in (select idNumber from vms19_suspects) and idProofType in (select idProofType from vms19_suspects)";
