...
class Suspect_model extends CI_Model {
    protected $tablename  = 'vms19_suspects';
    ...
    public function storeSuspectInfo($params) {
    ...
    public function updateSuspectInfo($update_data = array(), $aWhere = array()) {
    ...
    public function getSuspectInfo($aWhere) {
        if ( ! is_array($aWhere) || empty($aWhere)) {
            return array();
        }
    ...
    public function getAllSuspectInfo($aWhere = array()) {
        return $this->getAllRecords($this->tablename, $aWhere);
    }
    
    public function deleteSuspectInfo($aWhere = array()) {
        if ( ! is_array($aWhere) || empty($aWhere)) {
            return FALSE;
        }
    ...
    public function upload_image($filename, $source) {
        ...
        $object = $bucket->upload($file, [
            'name' => RELATIVE_SUSPECT_IMAGES_PATH . $filename
        ]);
    ...
    public function delete_image($filename) {
        $file = RELATIVE_SUSPECT_IMAGES_PATH . $filename;
        if (file_get_contents(SUSPECT_IMAGES_PATH.$filename)) {
            $storage = new StorageClient();
            $bucket = $storage->bucket(GCP_BUCKET_NAME);
